...
describe.only('------ Tayaba Flow ------', function () {
  ...
  const communityName2 = 'Rahat Community';
  const rahatToken1 = {
    name: 'H20 Wheels',
    symbol: 'H2W',
    decimals: 0,
  ...
  before(async function () {
    [deployer, donor, admin, manager, vendor1, vendor2, otpServer1, beneficiary1, beneficiary2] =
      ...
      const adminAddress = await ethers.getSigner(admin.address);
  ...
  describe('Upgrade the rahat community contract to new version', function () {
    it('Should upgrade the rahat community contract', async function () {
      const rahatAddress = rahatCommunity1.address;
      const RahatCommunity1 = await ethers.getContractFactory('RahatCommunity1',admin);
      rahatCommunity1 = await upgrades.upgradeProxy(rahatCommunity1.address,RahatCommunity1);
      expect(rahatCommunity1.address).to.equal(rahatAddress);
    });
    it('Should persist the state variables', async function () {
      expect(await rahatCommunity1.hasRole(vendorRole, vendor1.address)).to.equal(true);
      expect(await rahatCommunity1.isBeneficiary(beneficiary1.address)).to.equal(true);
      expect(await rahatCommunity1.name()).to.equal(communityName1);
    });
    it('Should be able to update the name state variable', async function () {
      await rahatCommunity1.connect(admin).changeName(communityName2);
      expect(await rahatCommunity1.name()).to.equal(communityName2);
